...
def main(args):
    ...
    # Plot
    PRR = args.PRR      # true                  # Plot Reproduce Result
    PRR_n_img_x = args.PRR_n_img_x              # number of images along x-axis in a canvas  10
    PRR_n_img_y = args.PRR_n_img_y              # number of images along y-axis in a canvas   10
    PRR_resize_factor = args.PRR_resize_factor  # resize factor for each image in a canvas   1
    ...
    x_hat = tf.placeholder(tf.float32, shape=[None, dim_img], name='input_img') #[none ,784]
    x = tf.placeholder(tf.float32, shape=[None, dim_img], name='target_img')     #[none ,784]

    # dropout
    keep_prob = tf.placeholder(tf.float32, name='keep_prob')

    # input for PMLR
    z_in = tf.placeholder(tf.float32, shape=[None, dim_z], name='latent_variable') #【none,20】
